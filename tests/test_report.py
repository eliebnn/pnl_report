...
def test_report_qty_price_date_ticker_one_name_1():
    ...
    df1['qty'] = [1, 7, -5, -2]
    df1['price'] = [10, 12, 8.5, 35.2]
    df1['date'] = ['2020-01-01', '2020-01-02', '2020-01-04', '2020-04-15']
    df1['ticker'] = ['RDSA', 'RDSA', 'RDSA', 'RDSA']

    report = PnLReport(data=df1, method='fifo').run()

    # Per Ticker Report

    df = pd.DataFrame.from_records(report.reports['RDSA'].pnls)
...
def test_report_qty_price_date_ticker_one_name_2():
    ...
    df = pd.DataFrame.from_records(report.reports['GOOG'].pnls)
